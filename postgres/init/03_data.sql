...
-- Nodes: Person
SELECT * FROM cypher('movies', $$ MERGE (n:Person {name: 'Alice', age: 30}) RETURN n $$) AS (n agtype);
SELECT * FROM cypher('movies', $$ MERGE (n:Person {name: 'Bob', age: 25}) RETURN n $$) AS (n agtype);
SELECT * FROM cypher('movies', $$ MERGE (n:Person {name: 'Carol', age: 35}) RETURN n $$) AS (n agtype);
SELECT * FROM cypher('movies', $$ MERGE (n:Person {name: 'Dave', age: 28}) RETURN n $$) AS (n agtype);

-- Nodes: Technology
SELECT * FROM cypher('movies', $$ MERGE (n:Technology {name: 'Python'}) RETURN n $$) AS (n agtype);
SELECT * FROM cypher('movies', $$ MERGE (n:Technology {name: 'Rust'}) RETURN n $$) AS (n agtype);
SELECT * FROM cypher('movies', $$ MERGE (n:Technology {name: 'PostgreSQL'}) RETURN n $$) AS (n agtype);
SELECT * FROM cypher('movies', $$ MERGE (n:Technology {name: 'Kubernetes'}) RETURN n $$) AS (n agtype);
